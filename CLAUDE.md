...
- **Attribution (default on; `--[no-]bundle-licenses`)**: every bundled file's leading header/license comments are hoisted to the top of the bundle in include/use encounter order (root first, deduplicated, **moved** not copied, cut at Customizer group markers), and one-line provenance banners (`// ======== include <lib.scad> ========`, `(continued)` on re-entry) separate the inlined sections (`Inlining/Attribution.cs`; SB5007 Info). Opt-in **`--parameters-first`** ([ADR 0002](docs/adr/0002-parameters-first-customizer-hoist.md)) emits this header block *below* the Customizer parameter prologue instead of above it — a comment-only (CSG-preserving) workaround for Thingiverse's Customizer, which fails to read parameters that follow a long leading comment block; `DeadCodeElimination` carries sticky leading trivia (the relocated header, the `/* [Hidden] */` fence) forward so it survives tree-shaking under `--minify`.
