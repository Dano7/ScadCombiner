...
/// <summary>
/// Golden-master tests over the on-disk parser corpus (<c>tests/Corpus/slice2-parser</c>). Each case
/// directory holds an <c>input.scad</c> and an <c>expected.ast</c> (the canonical <see cref="AstDump"/>
/// serialization of the parsed <c>ScadFile</c>). Set the <c>BLESS_AST</c> environment variable to
/// regenerate the golden files via <see cref="Regenerate"/>.
/// </summary>
public sealed class Slice2CorpusTests
{
    private const string SliceFolder = "slice2-parser";
